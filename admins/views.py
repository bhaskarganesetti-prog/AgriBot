...
def RegisterUsersView(request):
    ...
    if search_query:
        ...
        data = userProfile.objects.all().order_by('-id')  # Fetch all data if no search query
...
def activate_user(request, id):  # Accept id as a function parameter from URL
...
def BlockUser(request, id):  # Accept id as a function parameter
...
def UnblockUser(request, id):  # Accept id as a function parameter
...
def DeleteUser(request, id):
    print(f"DeleteUser: Received delete request for User ID = {id}")  # Debug

    if not id:
        print("DeleteUser: User ID is missing in the request.")  # Debug
        messages.error(request, "User ID is missing.")
        return redirect('RegisterUsersView')
    ...
    try:
        ...
        print(f"DeleteUser: Invalid User ID = {id}")  # Debug
        messages.error(request, "Invalid User ID.")
        return redirect('RegisterUsersView')

    user = get_object_or_404(userProfile, id=id)  # Get user or return 404
    user.delete()  # Delete user from database
    print(f"DeleteUser: User ID = {id} deleted successfully.")  # Debug
